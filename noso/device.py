...
def render_device_description(ctx: ServerContext, services: list[Service]) -> str:
    ...
    display_name = model.replace("Sonos ", "")  # "Sonos Connect" -> "Connect"
    friendly = f"{ip} - {model}"
    ...
    media_server = (
        ...
        f"<modelDescription>{esc(model)} Media Server</modelDescription>"
    ...
    media_renderer = (
        ...
        "<manufacturerURL>http://www.sonos.com</manufacturerURL>"
        f"<modelNumber>{esc(cfg.model_number)}</modelNumber>"
        f"<modelDescription>{esc(model)} Media Renderer</modelDescription>"
        f"<modelName>{esc(model)}</modelName>"
        "<modelURL>http://www.sonos.com</modelURL>"
